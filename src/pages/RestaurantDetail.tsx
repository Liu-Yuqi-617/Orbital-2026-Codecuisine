...
export default function RestaurantDetail() {
  ...
  const [restaurantInfo, setRestaurantInfo] =
    useState<Partial<SimpleRestaurant>>({});
  ...
  function loadRestaurantInfoFromStorage(
    restaurantId: string
  ): boolean {
    let hasInfo = false;

    const name =
      localStorage.getItem(`restaurant_name_${restaurantId}`) ||
      localStorage.getItem(`restaurant_${restaurantId}`) ||
      `Restaurant #${restaurantId}`;

    setRestaurantName(name);

    const infoStr = localStorage.getItem(
    ...
    if (infoStr) {
      try {
        const info =
          JSON.parse(infoStr) as Partial<SimpleRestaurant>;
    ...
    if (dateStr) {
      ...
      const wishlistData = localStorage.getItem(
        `wishlist_item_${restaurantId}`
      );
  ...
  async function fetchRestaurantInfoFromBackend(
    restaurantId: string
  ) {
    setLoadingInfo(true);
    ...
    try {
      ...
      if (placeId) {
        const response =
          await getRestaurantByPlaceId(placeId);

        if (response.data) {
          const data =
            response.data as SimpleRestaurant;
        ...
        console.warn(
          "No Google Place ID found for restaurant:",
          restaurantId
        );
        ...
        setRestaurantInfo({
          ...
          address:
            localStorage.getItem(
              `restaurant_address_${restaurantId}`
            ) || "",
          cuisine_type:
            localStorage.getItem(
              `restaurant_cuisine_${restaurantId}`
            ) || "",
        });
      ...
      console.error(
        "Error fetching restaurant info from backend:",
        error
      );
    } finally {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          alert(
            "GPS check-in successful! Review is now verified."
          );
  ...
  async function handleRemoveFromWishlist() {
    if (
      !confirm(
        "Remove this restaurant from wishlist?"
      )
    ) {
      return;
    }
          ...
          >
            <div style={{ flex: 1 }}>
              ...
              >
                  ...
                  restaurantInfo.cuisine_type !== "" && (
                    <span>
                      ...
                      {restaurantInfo.cuisine_type
                        .charAt(0)
                        .toUpperCase() +
                        restaurantInfo.cuisine_type.slice(
                          1
                        )}
                    </span>
                  )}

                {restaurantInfo.price_level &&
                  restaurantInfo.price_level > 0 && (
                    <span>
                      💰{" "}
                      {"$".repeat(
                        Math.min(
                ...
                {restaurantInfo.avg_taste !==
                  undefined &&
                  restaurantInfo.avg_taste > 0 && (
                    <span>
                      ⭐ Taste:{" "}
                      {restaurantInfo.avg_taste.toFixed(
                        1
                      )}
                    </span>
                  )}

                {restaurantInfo.avg_value !==
                  undefined &&
                  restaurantInfo.avg_value > 0 && (
                    <span>
                      💰 Value:{" "}
                      {restaurantInfo.avg_value.toFixed(
                        1
                      )}
                    </span>
                  )}

                {restaurantInfo.avg_ambiance !==
                  undefined &&
                  restaurantInfo.avg_ambiance > 0 && (
                    <span>
                      🏛️ Ambiance:{" "}
                      {restaurantInfo.avg_ambiance.toFixed(
                        1
                      )}
                    </span>
                  )}

                {restaurantInfo.total_review_count !==
                  undefined &&
                  restaurantInfo.total_review_count >
                    0 && (
                    <span
                ...
                {restaurantInfo.verified_review_count !==
                  undefined &&
                  restaurantInfo.verified_review_count >
                    0 && (
                    <span
